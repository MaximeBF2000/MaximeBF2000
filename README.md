# Hi there 👋 I'm Maxime

**Software Engineer | AI enthusiast | Entrepreneur**

> I do software engineering for fun and profit since 2018. <br>Currently deep in the worlds of AI coding, local-first development and MacOS dev.

*← btw thanks NanoBananaPro for the pic, I look much more chill in person*

### A few projects I've worked on / I work on 👇

- 🎬 [Momentum](https://github.com/MaximeBF2000/momentum-capture) - MacOS Screen recording app
- 💬 [OpenChatbot](https://github.com/MaximeBF2000/open-chatbot-electron) - Open-source desktop app to run local inference
- 📊 [SuperSimpleAnalytics](https://github.com/MaximeBF2000/super-simple-analytics) - Simpler than Google analytics, easily deploy on a VPS
- 🌐 [Chrome Network Analyzer](https://github.com/MaximeBF2000/network-analyzer-chrome-extension) - Chrome extension that adds a better Network panel in devtool, for reverse-engineers
- 📄 [docs-to-llm](https://github.com/MaximeBF2000/docs-to-llm) - Open-source CLI tool to convert any documentation into a llm.txt file
- 🗺️ [Google Maps Scraper for Desktop](https://github.com/MaximeBF2000/google-maps-reverse-engineered-electron-app) - A Desktop application that reverse-engineers Google maps for faster prospection
- 📧 [Minimailer](https://github.com/MaximeBF2000/minimailer-electron) - Desktop application to automate gmail outreach
- 🔍 [LinkedIn Profile Previewer Chrome](https://github.com/MaximeBF2000/linkedin-profil-previewer-chrome-extension) - Easily preview changes made on your profile before actual update
- 📺 [Youtube Short Disabler](https://github.com/MaximeBF2000/youtube-shorts-disabler-chrome-extension) - Remove shorts from your Youtube homepage
- 🐚 [My scripts](https://github.com/MaximeBF2000/personal-global-commands-for-macos) - My macos scripts, exported to path for fast access
- 📚 [ChatGPT Prompt Library Chrome](https://github.com/MaximeBF2000/gpt-prompt-library-chrome-extension) - Chrome extension that lets you save and manage prompts within the ChatGPT UI
- 📂 [FolderToASCIITree](https://github.com/MaximeBF2000/folder-to-ascii-tree) - CLI tool to create an ASCII tree design given a folder
- 💡 [ProductHunt Scraper](https://github.com/MaximeBF2000/producthunt-scraper) - Puppeteer automation to scrape ProductHunt products
- 👔 [LinkedIn AutoConnect](https://github.com/MaximeBF2000/linkedin-autoconnect-chrome-extension) - Chrome extension that automate sending of invites on the search page, by controlling the browser
- 🗺️ [Gmaps Scraper Chrome](https://github.com/MaximeBF2000/google-map-contact-scraper-chrome-extension) - Chrome extension to scrape Google Maps results
- ⭐️ [Malt Scraper Chrome](https://github.com/MaximeBF2000/malt-scraper-chrome-extension) - Chrome extension that scrapes Malt profiles and exports to CSV
- ⚡️ [Duplicate Chrome Tabs](https://github.com/MaximeBF2000/duplicate_chrome_tab_extension) - Chrome extension that adds "Option + D" shortcut to duplicate the focused tab
- 🅿️ [Pappers.fr Scraper Chrome](https://github.com/MaximeBF2000/pappersfr-scraper-chrome-extension) - Chrome extension that scrapes pappers.fr search results page
- 🤖 [CSV editor AI](https://github.com/MaximeBF2000/csv-ai-editor-cli) - CLI tool to edit a CSV with AI / talk to your CSV file
- ⛏️ [WelcomeToTheJungle Scraper](https://github.com/MaximeBF2000/welcome-to-the-jungle-scraper) - CLI tool to scrape welcometothejunlge.com + PHP webapp to explore and export subset of data


### The tech I'm used to 👇
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/jsLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/reactLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/nextjsLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/typescriptLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/mongodbLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/nodejsLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/sassLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/firebaseLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/pythonLogo.png" /> </a>
<a> <img height="40" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/netlifyLogo.png" /> </a>

### Find me here 👇
<a target="_blank" href="https://www.linkedin.com/in/maximebounaasferret/"> <img height="30" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/social_linkedin.png" /> </a>
<a target="_blank" href="https://www.malt.fr/profile/maximebounaas"> <img height="30" src="https://github.com/MaximeBF2000/MaximeBF2000/blob/master/social_malt.png" /> </a>
